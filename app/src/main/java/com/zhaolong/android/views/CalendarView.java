...
//			}
			drawCellText(canvas, i, date[i] + "", color,todayIndex);
		}
		super.onDraw(canvas);
	}
	
	private void drawTodayBg(Canvas canvas, int index, int color) {
		int x = getXByIndex(index);
		int y = getYByIndex(index);
		float left = surface.cellWidth * (x - 1) + surface.borderWidth;
		float top = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.borderWidth;
		float rad = surface.cellWidth>surface.cellHeight ? surface.cellHeight/2 : surface.cellWidth/2;
		paint.setColor(color); 
		paint.setStyle(Paint.Style.FILL); 
		/*canvas.drawCircle(left+surface.cellWidth/2+rad-4, top + surface.cellHeight/2-rad+4, 
				4, 
				paint);*/ 
	}
	...
	private void calculateDate() {
		...
		mLog.d(TAG, "day in week:" + dayInWeek);
		...
		// next month
		for (int i = monthStart + monthDay; i < date.length; i++) {
			date[i] = i - (monthStart + monthDay) + 1;
		}
		if (curEndIndex < date.length) {
			// 显示了下一月的
			calendar.add(Calendar.DAY_OF_MONTH, 1);
		}
		calendar.set(Calendar.DAY_OF_MONTH, date[date.length-1]);
	 ...
	 * @param text
	 */
	private void drawCellText(Canvas canvas, int index, String text, int color,int todayIndex) {
		int x = getXByIndex(index);
		int y = getYByIndex(index);
		...
		if (todayIndex != -1 && index == todayIndex) {
			//color = surface.todayNumberColor;
			drawTodayBg(canvas,index,surface.cellSelectedColor);
		}
	}
...
//				- surface.borderWidth, surface.cellBgPaint);
		paint.setColor(color); 
		paint.setStyle(Paint.Style.FILL); 
		canvas.drawCircle(left+surface.cellWidth/2, top + surface.cellHeight*2/3, 
				surface.cellWidth>surface.cellHeight ? surface.cellHeight/3 : surface.cellWidth/3, 
				paint); 
	}
	...
	private void drawDownOrSelectedBg(Canvas canvas) {
		...
		if (!selectedEndDate.before(showFirstDate)
			...
			if (section[1] == -1) {
				calendar.setTime(curDate);
				calendar.add(Calendar.MONTH, 1);
				findSelectedIndex(curEndIndex, date.length, calendar, section);
			}
			if (section[0] == -1) {
				section[0] = 0;
			}
			if (section[1] == -1) {
				section[1] = date.length-1;
			}
...
//		}
		...
		if (y > surface.monthHeight + surface.weekHeight) {
			...
			downIndex = (n - 1) * 7 + m - 1;
			mLog.d(TAG, "downIndex:" + downIndex);
			calendar.setTime(curDate);
	...
	private class Surface {
		...
		public void init() {
			...
			mLog.d(TAG, "text size:" + textSize);
			monthPaint.setTextSize(textSize);
			monthPaint.setTypeface(Typeface.DEFAULT_BOLD);
			weekPaint = new Paint();
			weekPaint.setColor(textColor);
			weekPaint.setAntiAlias(true);
			float weekTextSize = 34;
			weekPaint.setTextSize(weekTextSize);
			weekPaint.setTypeface(Typeface.DEFAULT_BOLD);
			datePaint = new Paint();
			datePaint.setColor(textColor);
			datePaint.setAntiAlias(true);
			float cellTextSize = 30;
