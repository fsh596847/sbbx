...
public class OrderWaitActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(device != null){
			...
			HlpUtils.setText(tvMan, device.getRepairName());
	...
	private void initViews() {
		...
		findViewById(R.id.tv_order_record).setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				startActivity(new Intent(OrderWaitActivity.this,RecordServiceActivty.class).putExtra("equipment", device.getEquipment()));
			}
		});
	}
	TextView tvRepairCode,tvEquipSeq,tvEquipBar,tvEquipName,tvHospital,tvDepar,tvEquipAddress,tvMan,tvTime;
						...
						if (hr != null){
							if  (hr.isSuccess()){
								...
								if(hintPopupWindow == null){
									hintPopupWindow = new HintPopupWindow(OrderWaitActivity.this,isAccept?"接单成功\n请按时到达客户现场":"您已拒绝此单\n此单将由后台客服处理","","确定");
